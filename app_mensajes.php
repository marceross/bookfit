...
function abrir_mensajes(conversacion)
{
  if($('#ocultos_'+conversacion).css('display')=='none')
  {
    $('#ocultos_'+conversacion).css('display', 'block');
    $('#botonflecha_'+conversacion).removeClass('badge-success');
    $('#botonflecha_'+conversacion).addClass('badge-warning');
    $('#flecha_'+conversacion).removeClass('down');
    $('#flecha_'+conversacion).addClass('up');

  }
  else
  {
    $('#ocultos_'+conversacion).css('display', 'none');
    $('#botonflecha_'+conversacion).removeClass('badge-warning');
    $('#botonflecha_'+conversacion).addClass('badge-success');    
    $('#flecha_'+conversacion).removeClass('up');
    $('#flecha_'+conversacion).addClass('down');
    //jquery
  }
}
...
<?php
 ...
 // {
   // $fecha_movimiento=$mensaje_fecha['fecha'];
    ...
    $msjes=mysqli_query($mysqli, "SELECT * FROM mensajes_conversaciones, mensajes WHERE mensajes_conversaciones.id_conversacion=mensajes.id_conversacion AND (id_usuario_rem='$id_usuario' or id_dest='$id_usuario') ORDER BY fecha_ultimo_msj DESC, fecha DESC, hora DESC");
...
// si el id_mensaje es el último de la conversación mostrar campo
      ...
      {      
        ...
        </div> <!-- Cierra el div que oculta mensajes -->
      </div>
      <div class="conversacion">
      <?php
      ...
      ?>
        ...
        <div class="row justify-content-start">
          ...
          <div class="col-5">
            <h6><span class="badge badge-light"><? echo $nombre_remitente;?></span>
              <? 
              if($mostrar_flecha==true)
              {
                $flecha='<a href="#" onClick="abrir_mensajes('.$msj['id_conversacion'].')"><p class="pflecha"><span id="flecha_'.$msj['id_conversacion'].'" class="arrow down"></span></p></a>';
              ...
              {
              ?>
                <span class="badge badge-info"><?echo " ". formato_latino($msj['fecha']);?></span>
                <span class="badge badge-light"><? echo " ". $msj['hora']." ";?></span>
                <span id="botonflecha_<? echo $msj['id_conversacion'];?>" class="badge badge-success"><? echo $flecha;?></span>
              <?
        ...
        <?php
        if($id_conversacion<>$msj['id_conversacion'] or $i==0)
        {
        ?>
          <div class="msjs_ocultos" style="display:none;" id="ocultos_<? echo $msj['id_conversacion'];?>">
        <?php
          $id_conversacion=$msj['id_conversacion'];
        }        
        ?>
